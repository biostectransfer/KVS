...
namespace KVSDataAccess.PDF
{
    ...
    public class LetterHead
    {
        ...
        public LetterHead(IEntities dbContext)
        {
            string topline = dbContext.GetSet<DocumentConfiguration>().Where(q => q.Id == "ADDRESS_TOPLINE").Select(q => q.Text).SingleOrDefault();
    ...
    public class HeaderFooter
    {
        ...
        internal virtual Table Write(MigraDoc.DocumentObjectModel.HeaderFooter hf)
        {
            ...
            if (this.Blocks.Count > 0)
            {
                ...
                var tableWidth = hf.Document.LastSection.PageSetup.PageWidth.Millimeter - hf.Document.LastSection.PageSetup.LeftMargin.Millimeter -
                hf.Document.LastSection.PageSetup.RightMargin.Millimeter;
    ...
    public abstract class AbstractPDFCreator
    {
        ...
        protected AbstractPDFCreator(IEntities dbContext, string logoFilePath, bool _isInvoice = false)
        {
            ...
            this.Header = new HeaderFooter()
            {
                ...
                LogoFilePath = dbContext.GetSet<DocumentConfiguration>().Where(q => q.Id == "LOGOPATH").Select(q => q.Text).SingleOrDefault(),
            ...
            if (IsInvoice)
            {
                block1 = dbContext.GetSet<DocumentConfiguration>().Where(q => q.Id == "FOOTER_BLOCK1").Select(q => q.Text).SingleOrDefault();
                block2 = dbContext.GetSet<DocumentConfiguration>().Where(q => q.Id == "FOOTER_BLOCK2").Select(q => q.Text).SingleOrDefault();
                block3 = dbContext.GetSet<DocumentConfiguration>().Where(q => q.Id == "FOOTER_BLOCK3").Select(q => q.Text).SingleOrDefault();
            }
        ...
        protected Table GetTableFromDataTable(DataTable dt, List<int> columnWidths, List<int> shadedColumns, bool stretchToPageWidth)
        {
            ...
            {
                double pageWidth = this.Document.LastSection.PageSetup.PageWidth.Millimeter - this.Document.LastSection.PageSetup.LeftMargin.Millimeter -
                    this.Document.LastSection.PageSetup.RightMargin.Millimeter;
